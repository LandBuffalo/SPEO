...
double CEC2014::EvaluateFitness(double * elements)
{
	...
	case 3:
		...
		break;
	case 4:
		rosenbrock_func(elements, &fitness_value, dim_, OShift, Mdata, 1, 1, 1, 2.048 / (double)100);
		fitness_value += 400.0;
		break;
	case 5:
		...
		break;
	case 6:
		weierstrass_func(elements, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 0.5 / (double)100);
		fitness_value += 600.0;
		break;
	case 7:
		...
		break;
	case 8:
		rastrigin_func(elements, &fitness_value, dim_, OShift, Mdata, 1, 0, 0, 5.12 / (double)100);
		fitness_value += 800.0;
		break;
	case 9:
		rastrigin_func(elements, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 5.12 / (double)100);
		fitness_value += 900.0;
		break;
	case 10:
	...
	case 11:
		...
		break;
	case 12:
		katsuura_func(elements, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 5 / (double)100);
		fitness_value += 1200.0;
		break;
	case 13:
		happycat_func(elements, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 5 / (double)100);
		fitness_value += 1300.0;
		break;
	case 14:
		hgbat_func(elements, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 5 / (double)100);
		fitness_value += 1400.0;
		break;
	case 15:
		grie_rosen_func(elements, &fitness_value, dim_, OShift, Mdata, 1, 1, 1, 5 / (double)100);
...
double CEC2014::EvaluateFitness(const vector<double> & elements)
{
	...
	case 3:
		...
		break;
	case 4:
		rosenbrock_func(pop_original_, &fitness_value, dim_, OShift, Mdata, 1, 1, 1, 2.048 / (double)100);
		fitness_value += 400.0;
		break;
	case 5:
		...
		break;
	case 6:
		weierstrass_func(pop_original_, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 0.5 / (double)100);
		fitness_value += 600.0;
		break;
	case 7:
		...
		break;
	case 8:
		rastrigin_func(pop_original_, &fitness_value, dim_, OShift, Mdata, 1, 0, 0, 5.12 / (double)100);
		fitness_value += 800.0;
		break;
	case 9:
		rastrigin_func(pop_original_, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 5.12 / (double)100);
		fitness_value += 900.0;
		break;
	case 10:
	...
	case 11:
		...
		break;
	case 12:
		katsuura_func(pop_original_, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 5 / (double)100);
		fitness_value += 1200.0;
		break;
	case 13:
		happycat_func(pop_original_, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 5 / (double)100);
		fitness_value += 1300.0;
		break;
	case 14:
		hgbat_func(pop_original_, &fitness_value, dim_, OShift, Mdata, 1, 1, 0, 5 / (double)100);
		fitness_value += 1400.0;
		break;
	case 15:
		grie_rosen_func(pop_original_, &fitness_value, dim_, OShift, Mdata, 1, 1, 1, 5 / (double)100);
		fitness_value += 1500.0;
		break;
	case 16:
...
int CEC2014::Initilize(int func_id, int dim)
{
	...
	{
		Mdata = new double[dim_ * dim_ * cf_func_num_];
		if (Mdata == NULL)
			printf("\nError: there is insufficient memory available!\n");
	...
	{
		OShift = new double[dim_ * cf_func_num_];
		if (OShift == NULL)
			printf("\nError: there is insufficient memory available!\n");
...
void CEC2014::sphere_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Sphere */
{
	int i;
	f[0] = 0.0;
...
void CEC2014::ellips_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Ellipsoidal */
{
	int i;
	f[0] = 0.0;
	sr_func(x, ze, nx, Os, Mr, rate, s_flag, r_flag); /* shift and rotate */
	...
	for (i = 0; i<nx; i++)
	{
		double tmp = pow(10.0, 6.0*i / (nx - 1))*ze[i] * ze[i];
		f[0] += tmp;
	}
}

void CEC2014::bent_cigar_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Bent_Cigar */
{
	int i;
	sr_func(x, ze, nx, Os, Mr, rate, s_flag, r_flag); /* shift and rotate */
...
void CEC2014::discus_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Discus */
{
	int i;
	sr_func(x, ze, nx, Os, Mr, rate, s_flag, r_flag); /* shift and rotate */
...
void CEC2014::dif_powers_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Different Powers */
{
	int i;
	f[0] = 0.0;
	sr_func(x, ze, nx, Os, Mr, rate, s_flag, r_flag); /* shift and rotate */
...
void CEC2014::rosenbrock_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Rosenbrock's */
{
	int i;
	double tmp1, tmp2;
	f[0] = 0.0;
...
void CEC2014::schaffer_F7_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Schwefel's 1.2  */
{
	int i;
	double tmp;
	f[0] = 0.0;
...
void CEC2014::ackley_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Ackley's  */
{
	int i;
	double sum1, sum2;
	sum1 = 0.0;
...
void CEC2014::weierstrass_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Weierstrass's  */
{
	int i, j, k_max;
	double sum, sum2, a, b;
	a = 0.5;
...
void CEC2014::griewank_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Griewank's  */
{
	int i;
	double s, p;
	s = 0.0;
...
void CEC2014::rastrigin_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Rastrigin's  */
{
	int i;
	f[0] = 0.0;
...
void CEC2014::step_rastrigin_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Noncontinuous Rastrigin's  */
{
	int i;
	f[0] = 0.0;
...
void CEC2014::schwefel_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Schwefel's  */
{
	int i;
	double tmp;
	f[0] = 0.0;
...
void CEC2014::katsuura_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Katsuura  */
{
	int i, j;
...
void CEC2014::bi_rastrigin_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Lunacek Bi_rastrigin Function */
{
	int i;
...
void CEC2014::grie_rosen_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Griewank-Rosenbrock  */
{
	int i;
...
void CEC2014::escaffer6_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* Expanded Scaffer??s F6  */
{
	int i;
	double temp1, temp2;
...
void CEC2014::happycat_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* HappyCat, provdided by Hans-Georg Beyer (HGB) */
...
void CEC2014::hgbat_func(double *x, double *f, int nx, double *Os, double *Mr, int s_flag, int r_flag, double fixedShift, double rate) /* HGBat, provdided by Hans-Georg Beyer (HGB)*/
/* original global optimum: [-1,-1,...,-1] */
{
	int i;
	double alpha, r2, sum_z;
...
void CEC2014::hf01(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 1 */
{
	int i, tmp, cf_num = 3;
	double fit[3];
	int G[3], G_nx[3];
...
void CEC2014::hf02(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 2 */
{
	int i, tmp, cf_num = 3;
	double fit[3];
	int G[3], G_nx[3];
...
void CEC2014::hf03(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 3 */
{
	int i, tmp, cf_num = 4;
	double fit[4] = { 0 };
	int G[4], G_nx[4];
...
void CEC2014::hf04(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 4 */
{
	int i, tmp, cf_num = 4;
	double fit[4] = { 0 };
	int G[4], G_nx[4];
...
void CEC2014::hf05(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 5 */
{
	int i, tmp, cf_num = 5;
	double fit[5] = { 0 };
	int G[5], G_nx[5];
...
void CEC2014::hf06(double *x, double *f, int nx, double *Os, double *Mr, int *S, int s_flag, int r_flag) /* Hybrid Function 6 */
{
	int i, tmp, cf_num = 5;
	double fit[5] = { 0 };
	int G[5], G_nx[5];
	double Gp[5] = { 0.1, 0.2, 0.2, 0.2, 0.2 };
...
void CEC2014::cf01(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 1 */
{
	int i, cf_num = 5;
	double fit[5];
...
void CEC2014::cf02(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 2 */
{
	int i, cf_num = 3;
	double fit[3];
...
void CEC2014::cf03(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 3 */
{
	int i, cf_num = 3;
	double fit[3];
...
void CEC2014::cf04(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 4 */
{
	int i, cf_num = 5;
	double fit[5];
...
void CEC2014::cf05(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 4 */
{
	int i, cf_num = 5;
	double fit[5];
...
void CEC2014::cf06(double *x, double *f, int nx, double *Os, double *Mr, int r_flag) /* Composition Function 6 */
{
	int i, cf_num = 5;
	double fit[5];
	double delta[5] = { 10, 20, 30, 40, 50 };
...
void CEC2014::cf07(double *x, double *f, int nx, double *Os, double *Mr, int *SS, int r_flag) /* Composition Function 7 */
{
	int i, cf_num = 3;
	double fit[3];
	double delta[3] = { 10, 30, 50 };
...
void CEC2014::cf08(double *x, double *f, int nx, double *Os, double *Mr, int *SS, int r_flag) /* Composition Function 8 */
{
	int i, cf_num = 3;
	double fit[3];
	double delta[3] = { 10, 30, 50 };
...
void CEC2014::shiftfunc(double *x, double *xshift, int nx, double *Os)
{
	int i;
	for (i = 0; i<nx; i++)
	{
...
void CEC2014::rotatefunc(double *x, double *xrot, int nx, double *Mr)
{
	int i, j;
	for (i = 0; i<nx; i++)
	{
...
void CEC2014::sr_func(double *x, double *sr_x, int nx, double *Os, double *Mr, double sh_rate, int s_flag, int r_flag) /* shift and rotate */
{
	int i;
	if (s_flag == 1)
	{
...
void CEC2014::asyfunc(double *x, double *xasy, int nx, double beta)
{
	int i;
	for (i = 0; i<nx; i++)
	{
...
void CEC2014::oszfunc(double *x, double *xosz, int nx)
{
	int i, sx;
	double c1, c2, xx;
	for (i = 0; i<nx; i++)
...
void CEC2014::cf_cal(double *x, double *f, int nx, double *Os, double * delta, double * bias, double * fit, int cf_num)
{
	int i, j;
	double *w;
