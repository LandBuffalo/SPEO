...
{
    file_name_ = "./Results/SPEO_CPU.csv";
    ifstream exist_file;
...
int IslandEA::Initilize(IslandInfo island_info, ProblemInfo problem_info)
{
    ...
    EA_CPU_ = new DE_CPU();
    EA_CPU_->Initilize(problem_info_, island_info_);
...
int IslandEA::Execute()
{
...
#endif   
    {
		...
		if (generation % island_info_.interval == 0 && node_info_.node_num > 1)
        {
        	double tmp_time = MPI_Wtime();
            migrate_.MigrateIn(sub_population_);
            communication_time += (real) (MPI_Wtime() - tmp_time);
        }
        double tmp_time = MPI_Wtime();
        if (node_info_.node_num > 1)
...
int IslandEA::RecordDisplayUnit(real current_time, real communication_time)
{
    ...
    display_unit_.fitness_value = EA_CPU_->FindBestIndividual(sub_population_).fitness_value;
    return 0;
}
...
int IslandEA::MergeResults(vector<DisplayUnit> &total_display_unit)
{
    ...
    display_unit_.time = display_unit_.time / (node_info_.node_num + 0.0);
    display_unit_.communication_percentage = display_unit_.communication_percentage / (node_info_.node_num + 0.0);
...
int IslandEA::PrintResult()
{
    ...
    string EA_parameters = EA_CPU_->GetParameters();
    ...
    <<','<<node_info_.GPU_num<<','<<island_info_.island_num<<','<<total_pop_size<<','<<island_info_.island_size<<','<<island_info_.interval<<','<<island_info_.migration_size\
...
#else
	file<<function_ID<<','<<run_ID<<','<<dim<<','<<fixed<<setprecision(15)<<best_fitness<<','<<time_period<<','<<communication_percentage<<','<<total_FEs<<','<<node_info_.node_num\
    <<','<<node_info_.GPU_num<<','<<island_info_.island_num<<','<<total_pop_size<<','<<island_info_.island_size<<','<<island_info_.interval<<','<<island_info_.migration_size\
	<<','<<island_info_.migration_rate<<','<<island_info_.buffer_capacity<<','<<island_info_.migration_topology<<','<<island_info_.buffer_manage<<','<<island_info_.regroup_option<<','<<island_info_.configure_EA<<','<<EA_parameters<<endl;
